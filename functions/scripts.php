...
add_action('login_enqueue_scripts', 'devq_login_assets');


function devq_login_header_url()
{
  return 'https://thedevq.com/';
}
add_filter('login_headerurl', 'devq_login_header_url');


function devq_login_header_text()
{
  return get_bloginfo('name');
}
add_filter('login_headertext', 'devq_login_header_text');


function devq_login_footer_credit()
{
  echo '<div class="devq-login-credit">Yeti by Darin S.</div>';
}
add_action('login_footer', 'devq_login_footer_credit');


function devq_login_yeti($message)
{
  ob_start();
  ?>
  <div class="devq-login-logo">
    <a href="https://thedevq.com/" target="_blank" rel="noopener">
      <img src="<?php echo esc_url(get_template_directory_uri() . '/images/loginlogo.png'); ?>" alt="<?php echo esc_attr(get_bloginfo('name')); ?>" />
    </a>
  </div>
...
add_filter('login_message', 'devq_login_yeti');
